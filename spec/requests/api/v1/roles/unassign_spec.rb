describe 'GET api/v1/roles', type: :request do
  ...
  subject { post unassign_api_v1_role_path(role), params: params, headers: headers, as: :json }
  ...
  context 'when role has one permission assigned' do
    ...
    it 'Increases the number of RolePermission' do
      expect { subject }.to change { RolePermission.count }.from(1).to(0)
    end

    it 'Increases the permissions of the role' do
      expect { subject }.to change { role.permissions.count }.from(1).to(0)
    end
  end
  ...
  context 'when role has all permissions assigned' do
    ...
    it 'Decreases the number of RolePermission' do
      expect { subject }.to change { RolePermission.count }.from(2).to(0)
    end

    it 'Decreases the permissions of the role' do
      expect { subject }.to change { role.permissions.count }.from(2).to(0)
    end
  end
  ...
  context 'When the role doesnt have any permissions' do
    ...
    it 'Doesnt change the number of RolePermission' do
      expect { subject }.not_to change { RolePermission.count }
    end

    it 'Doesnt change the permissions of the role' do
